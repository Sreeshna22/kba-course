...
adminauth.put("/updateCourse", authenticate, adminCheck, async (req, res) => {
    try {
        ...
        const result = await Course.findOne({ courseName: new RegExp(`^${CourseName}$`, "i") }).exec();
        ...
        result.courseType = CourseType || result.courseType;
        result.description = Description || result.description;
        result.price = Price || result.price;

        await result.save();

        res.status(200).json({ message: "Course successfully updated", data: result });
    } catch (err) {
